    ...
    pub fn deinit(self: *Result, alloc: Allocator) void {
        switch (self.*) {
            .completed => |completed| if (completed.ownership == .owned) {
                ...
                if (completed.completion.reasoning) |reasoning| alloc.free(@constCast(reasoning));
                if (completed.completion.reasoning_signature) |signature| alloc.free(@constCast(signature));
                if (completed.completion.reasoning_item_id) |id| alloc.free(@constCast(id));
                if (completed.completion.reasoning_encrypted_content) |content| alloc.free(@constCast(content));
                types.freeResponsesReasoningItems(alloc, completed.completion.reasoning_items);
                types.freeResponsesProviderOutputItems(alloc, completed.completion.responses_provider_output_items);
                types.freeResponsesUrlCitations(alloc, completed.completion.url_citations);
...
test "owned stream result releases every Responses completion field" {
    ...
    var result: Result = .{ .completed = .{
        .completion = .{
            .content = try alloc.dupe(u8, "answer"),
            .reasoning = try alloc.dupe(u8, "reasoning"),
            .reasoning_signature = try alloc.dupe(u8, "signature"),
            .reasoning_item_id = try alloc.dupe(u8, "rs_1"),
            .reasoning_encrypted_content = try alloc.dupe(u8, "opaque"),
            .reasoning_items = try types.dupeResponsesReasoningItems(alloc, &.{.{
                .output_index = 0,
                .id = "rs_1",
                .summary = "summary",
                .encrypted_content = "opaque",
            }}),
            .responses_provider_output_items = try types.dupeResponsesProviderOutputItems(alloc, &.{.{
                .output_index = 0,
                .json = "{\"type\":\"reasoning\"}",
            }}),
            .url_citations = try types.dupeResponsesUrlCitations(alloc, &.{.{
