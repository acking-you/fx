...
test "post-tool decision prompt appends one no-cache user message only when pending" {
...
fn completionStepReasoning(completion: types.ModelCompletion) runtime_tool_batch.StepReasoning {
    return .{
...
fn materializeConfirmedProviderTools(
    ...
    try runtime_tool_batch.appendAssistantToolCallStepWithReasoning(
        ...
        null,
        novel_calls,
        completionStepReasoning(completion),
...
fn processQueuedPromptLoop(
    ...
    while (agent_steps.allowsStep(config.agent_step_limit, step)) : (step += 1) {
        ...
        const applied_steer_count = try drainPendingSteers(
            deps,
            arena,
            turn_id,
            &within_turn_suffix,
        ...
        while (true) {
            ...
            const projected_request_messages = blk: {
                ...
                if (inline_compaction != null) break :blk decision_source_messages;
                const user_message_index = current_user_message_index orelse
                    return error.MissingCurrentUserMessage;
            ...
            if (inline_auto_compaction_pending) {
                ...
                var compaction_tools = try responses_request_protocol.prepareTools(
                    overlay_arena,
                    .{
            ...
            stream_result = runtime_gateway_step.streamModelCompletion(
                ...
                const codex_credential_changed_before_send =
                    ...
                    job.credential_source.? == .chatgpt_subscription and
                    ...
                    ))
                {
                    ...
                    debug_trace.eventf(
                        ...
                        step_ctx,
                        "source=chatgpt_subscription delivery=definitely_unsent",
                        .{},
                    );
            ...
            if (response_failure) |failure| {
                if (!context_overflow_recovery_used and
                    ...
                    streamReplaySafe(&stream_ctx))
                {
                    debug_trace.eventf(
                        ...
                        .{@intFromEnum(failureHttpStatus(failure.kind))},
                    );
                    stream_result.deinit(arena);
                    stream_result_set = false;
