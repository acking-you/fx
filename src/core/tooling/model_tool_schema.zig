...
/// Serializes a provider-neutral function selection as the flattened JSON
/// array accepted by the shared Responses request builder.
pub fn functionSchemaArrayJsonAlloc(
    alloc: std.mem.Allocator,
    schemas: []const FunctionSchema,
) ![]u8 {
    var out: std.Io.Writer.Allocating = .init(alloc);
    errdefer out.deinit();
