...
pub const gateway_auth_file_name = "gateway-auth.json";
...
pub fn gatewayAuthPath(alloc: Allocator, home: []const u8) ![]u8 {
    return std.fs.path.join(alloc, &.{ home, root_dir_name, gateway_auth_file_name });
}
...
test "profile path helpers preserve current default locations" {
    ...
    const gateway_auth = try gatewayAuthPath(alloc, "/tmp/fake-home");
    defer alloc.free(gateway_auth);
    try std.testing.expectEqualStrings("/tmp/fake-home/.fx/gateway-auth.json", gateway_auth);
