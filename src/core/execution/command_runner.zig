...
test "explicit captured profiles execute the selected shells" {
    ...
    if (std.Io.Dir.accessAbsolute(io_mod.getIo(), "/bin/zsh", .{})) {
        ...
        try std.testing.expectEqual(@as(?i64, 0), zsh_user.command_result.?.foreground.exit_code);
        try std.testing.expect(std.mem.find(u8, zsh_user.output, "profile-stdout") != null);
        try std.testing.expect(std.mem.find(u8, zsh_user.output, "profile-stderr") != null);
    } else |_| {}
}
