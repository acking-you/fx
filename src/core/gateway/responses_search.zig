...
// Generated from the latest Codex `SearchCommands` schemars contract and
// `ext/web-search/web_run_description.md`. ChatGPT treats `web.run` as a
// reserved function and rejects even semantically compatible declarations
// whose description or JSON Schema differs from this configured definition.
const codex_web_run_namespace_json = @embedFile("codex_web_run_tool.json");

/// Builds the latest Codex standalone web namespace declaration. The returned
/// JSON is one Responses tool object and is owned by the caller.
pub fn buildNamespaceToolAlloc(alloc: Allocator) ![]u8 {
...
test "Responses standalone search tool declares the latest command surface" {
    const tool = try buildNamespaceToolAlloc(std.testing.allocator);
...
fn checkAllocationFailures(alloc: Allocator) !void {
    const tool = try buildNamespaceToolAlloc(alloc);
