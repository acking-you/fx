...
pub const slash_specs = [_]SlashSpec{
    ...
    .{ .kind = .background, .command = "/background", .aliases = &.{"/ps"}, .help_entry = "/background (/ps) [stop <command-id|last>]", .completion_description = "inspect yielded commands and hosted terminals", .presentation_category = .agents, .show_in_welcome = true, .has_args = true },
    ...
    .{ .kind = .bash_first, .command = "/bash-first", .help_entry = "/bash-first [on|off|auto]", .completion_description = "prefer shell and rg for workspace search (auto follows permission mode)", .presentation_category = .model, .has_args = true, .accepts_payload = true },
    .{ .kind = .exec_mode, .command = "/exec-mode", .help_entry = "/exec-mode [codex|claude]", .completion_description = "choose long-command continuation behavior", .presentation_category = .model, .has_args = true, .accepts_payload = true },
