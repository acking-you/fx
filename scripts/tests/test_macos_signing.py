...
SIGNING_IDENTITY = "Developer ID Application: Example Developer (EXAMPLE123)"
SIGNING_IDENTITY_HASH = "0123456789ABCDEF0123456789ABCDEF01234567"
...
if args and args[0] == "find-identity":
    print('  1) {SIGNING_IDENTITY_HASH} "{SIGNING_IDENTITY}"')
...
''',
    ...
    def test_signs_notarizes_and_cleans_credentials_without_printing_secrets(
        ...
        with tempfile.TemporaryDirectory(prefix="fx-macos-signing-test-") as tmp:
            ...
            self.assertIn(f"--sign {SIGNING_IDENTITY_HASH}", events)
            self.assertIn("--identifier fx", events)
    ...
    def test_rejects_a_signature_without_an_apple_team(self) -> None:
        self.assertTrue(SCRIPT_PATH.is_file(), "macOS signing helper is missing")
        with tempfile.TemporaryDirectory(prefix="fx-macos-signing-test-") as tmp:
            root = pathlib.Path(tmp)
            ...
            result, _, runner_temp, _ = self.run_script(
                root,
                {"FX_SIGNING_TEST_TEAM_ID": ""},
            )

            output = result.stdout + result.stderr
            self.assertNotEqual(0, result.returncode, output)
            self.assertIn("missing its team identifier", output)
            self.assertEqual([], list(runner_temp.iterdir()))
...
class MacosSigningWorkflowTests(unittest.TestCase):
    ...
    def test_stable_release_is_the_only_workflow_with_signing_secrets(self) -> None:
        ...
        for workflow_path in (REPO_ROOT / ".github" / "workflows").glob("*.yml"):
            ...
            for secret_name in SECRET_NAMES:
                self.assertNotIn(secret_name, workflow, workflow_path.name)
            self.assertNotIn("sign-and-notarize-macos", workflow, workflow_path.name)
