...
pub fn buildRequest(
    ...
    try writeInput(writer, alloc, request);
    ...
    for (request.messages, 0..) |message, message_index| {
        switch (message.role) {
            .system => {
                ...
                try writeCompactionReplayInput(writer, alloc, checkpoint.input_json, &first);
            },
            .user => {
                ...
                if (request.verified_images) |images| {
                    if (message_index == request.messages.len - 1) {
                        for (images) |image| {
...
fn matchingCompactionCheckpoint(
    ...
    if (source != .chatgpt_subscription or source != checkpoint.credential_source) return null;
    if (!std.mem.eql(u8, request.model, checkpoint.wire_model)) return null;
    ...
    responses_compaction_binding.validate(source, request_binding) catch return null;
    responses_compaction_binding.validate(source, checkpoint_binding) catch return null;
    if (!responses_compaction_binding.eql(request_binding, checkpoint_binding)) return null;
    return checkpoint;
}

fn writeCompactionReplayInput(
    writer: *std.Io.Writer,
    alloc: Allocator,
    input_json: []const u8,
    first: *bool,
) !void {
    try responses_compaction.validateReplayInputJson(alloc, input_json);
    var parsed = std.json.parseFromSlice(std.json.Value, alloc, input_json, .{}) catch |err| switch (err) {
        error.OutOfMemory => return error.OutOfMemory,
        else => return error.InvalidResponsesCompactInput,
    };
    defer parsed.deinit();
...
test "OpenAI Codex transport declares direct usage accounting" {
