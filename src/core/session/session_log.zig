...
pub const Root = struct {
    ...
    pub fn initFromHome(
        ...
        if (mode == .writable) {
            io_mod.setDirPermissions(durable_home, private_dir_permissions) catch
        ...
        if (mode == .writable) {
            io_mod.setDirPermissions(sessions_dir, private_dir_permissions) catch
                return error.PrivateStatePermissionsUnsupported;
        }
...
fn openSessionDir(
    ...
    if (mode == .writable) {
        io_mod.setDirPermissions(dir, private_dir_permissions) catch
            return error.PrivateStatePermissionsUnsupported;
    }
...
test "torn exact settlement restores stale sidecar backlog over settled rollback" {
    ...
    defer initial.deinit(alloc);
    {
        ...
        try bridge_observation.complete(alloc, "codex/gpt-test", completion, .{ .exact = .codex });
        ...
        try settled_observation.complete(alloc, "codex/gpt-test", completion, .{ .exact = .codex });
