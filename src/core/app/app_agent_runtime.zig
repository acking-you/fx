    ...
    return struct {
        ...
        fn toolContextWithAuthority(
            ...
            if (comptime @hasField(App, "web_search_runtime")) {
                ...
                if (model_provider.usesGatewayAuxiliaries(active_provider) or active_provider == .codex) {
                ...
                ctx.web_search_runtime_ready = active_provider == .codex;
...
test "app Codex route configures Codex web search without a Gateway reviewer" {
    const alloc = std.testing.allocator;
    var app = try FakeApp.init(alloc);
    defer app.deinit();
    var credential = credentials.Credential{
