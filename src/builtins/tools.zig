...
const unified_exec_impl = @import("../tools/shell/unified_exec.zig");
...
pub const exec_command = ToolSpec{
    .name = "exec_command",
    ...
    .decode = unified_exec_impl.decodeExec,
    .validate = unified_exec_impl.validateExec,
    .call = unified_exec_impl.callExec,
    .reads_only_fn = unified_exec_impl.readsOnly,
    .irreversible_fn = unified_exec_impl.irreversible,
};

pub const write_stdin = ToolSpec{
    .name = "write_stdin",
    ...
    .executor_kind = .write_stdin,
    .activity_kind = .command,
    .requires_approval = false,
    .action_label = "Polling",
    ...
    .call = unified_exec_impl.callWrite,
    .reads_only_fn = unified_exec_impl.readsOnly,
    .irreversible_fn = unified_exec_impl.irreversible,
};
...
test "built-in tool lookup and metadata use registered defaults" {
    const spec = lookup("exec_command") orelse return error.TestExpectedEqual;
    try std.testing.expectEqual(tool_specs.ExecutorKind.exec_command, spec.executor_kind);
    try std.testing.expectEqual(types.ToolActivityKind.command, toolActivityKind("exec_command"));
    try std.testing.expect(toolRequiresApproval("exec_command"));
    try std.testing.expect(toolHasPermissionContract("exec_command"));
    try std.testing.expect(lookup("write_stdin") != null);
    try std.testing.expect(lookup("terminal") == null);
...
test "built-in subagent owns product metadata schema and callbacks" {
    ...
    try std.testing.expect(std.mem.find(u8, subagent.description, "use inspect.wait instead of exec_command") != null);
...
test "built-in skill registry order follows exec tools" {
    var exec_pos: ?usize = null;
    var skill_pos: ?usize = null;
    for (all, 0..) |tool, index| {
        if (std.mem.eql(u8, tool.name, "exec_command")) exec_pos = index;
        if (std.mem.eql(u8, tool.name, "skill")) skill_pos = index;
    }

    try std.testing.expect(exec_pos != null);
    try std.testing.expect(skill_pos != null);
    try std.testing.expect(exec_pos.? < skill_pos.?);
}
