...
fn loadedCatalogStatusText(state: model_cache_runtime.ModelMenuCatalogState) ?[]const u8 {
    ...
    if (state.private_models_hidden) {
        ...
        return switch (reason) {
            .no_credential => "Using the public model catalog; configure a BYOK credential to load authenticated models.",
            .credential_refresh_failed => "The selected credential could not refresh; using the public model catalog.",
        };
    }
...
fn retryableFailureText(failure: ?model_cache_runtime.ModelMenuCatalogState.Failure) ?[]const u8 {
    ...
    return switch (value.category) {
        ...
        .transport, .gateway_unavailable => "Could not reach the model provider; retry /model.",
...
test "model menu states and navigation budget stay bounded" {
    ...
    try std.testing.expect(std.mem.find(u8, failed_state.items, "Could not reach the model provider; retry /model.") != null);
...
test "model menu places catalog note after an item gap and preserves the heading gap" {
    ...
    defer status.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, status.items, "authenticated with an API key") != null);
}

test "model menu status follows provenance and retryable failure precedence" {
    try std.testing.expectEqualStrings(
        "OpenAI-compatible catalog: authenticated with an API key.",
    ...
    const cases = [_]struct {
        ...
        .{ .state = .{ .public_only_reason = .no_credential, .private_models_hidden = true }, .expected = "Using the public model catalog; configure a BYOK credential to load authenticated models." },
        .{ .state = .{ .public_only_reason = .credential_refresh_failed, .private_models_hidden = true }, .expected = "The selected credential could not refresh; using the public model catalog." },
        .{ .state = .{ .failure = .{ .category = .transport, .retryable = true } }, .expected = "Could not reach the model provider; retry /model." },
