...
pub const QueuedPrompt = struct {
    ...
    api_key: []u8,
    /// Owned with api_key at admission, including while compaction holds the queue.
    gateway_endpoint: ?[]u8 = null,
...
pub const WorkerRuntime = struct {
    ...
    pub fn takePendingSteer(
        ...
        for (self.queued_prompts.items, 0..) |queued, index| {
            if (queued.steer_target_turn_id == null or
                queued.steer_target_turn_id.? != turn_id)
            {
                continue;
            }
    ...
    pub fn queuePreview(self: *WorkerRuntime) QueuePreview {
        ...
        if (self.active_turn_id != 0) {
            for (self.queued_prompts.items) |queued| {
                if (queued.steer_target_turn_id == self.active_turn_id) steering_count += 1;
            }
        }
        return .{
            .count = count,
...
pub fn freeQueuedPrompt(alloc: std.mem.Allocator, prompt: QueuedPrompt) void {
    ...
    if (prompt.gateway_endpoint) |endpoint| alloc.free(endpoint);
...
test "input admitted during processing steers the active turn" {
    ...
    try std.testing.expectEqual(@as(?u64, null), steer.steer_target_turn_id);
...
test "unconsumed steer falls back to the next queued turn" {
    ...
    try std.testing.expectEqual(@as(?u64, null), follow_up.steer_target_turn_id);
}
...
test "queue review atomically blocks steering consumption and edits by prompt identity" {
    ...
    try std.testing.expect((try runtime.takePendingSteer(alloc, 41)) == null);
    ...
    const guidance = (try runtime.takePendingSteer(alloc, 41)) orelse
        return error.TestExpectedEqual;
    defer freeQueuedPrompt(alloc, guidance);
    try std.testing.expectEqualStrings("after", guidance.prompt);
}
...
test "clear queued prompts also clears steering" {
    ...
    try std.testing.expectEqual(@as(usize, 0), runtime.queuedPromptCount());
    try std.testing.expect((try runtime.takePendingSteer(alloc, 9)) == null);
}
...
test "clear queued prompts preserves events and discard frees event payloads" {
    ...
    var owned_route_prompt = try makePrompt(alloc, "drop", "model");
    owned_route_prompt.gateway_endpoint = try alloc.dupe(u8, "https://queued.example.test/v1/responses");
    try runtime.enqueuePrompt(alloc, owned_route_prompt);
    try runtime.pushEvent(alloc, .{ .assistant_presentation = .{
        .text = @constCast("kept"),
    } });
    runtime.clearQueuedPrompts(alloc, &.{});
