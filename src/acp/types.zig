...
pub fn writeInitializeResponseWithOptions(
    ...
    if (options.provider_control_capable) {
        try w.writeAll(",\"providerControl\":{\"switch\":true,\"login\":true,\"loginMethods\":[\"device_code\",\"browser\"],\"setup\":true,\"configureByok\":true,\"usage\":true,\"status\":true,\"logout\":true,\"refresh\":true}");
    }
    try w.writeAll("}}}");
}
