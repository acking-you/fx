...
test "runtime context ordering" {
    var fixture = PromptContextFixture{ .project_context = "project facts" };

    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    var messages: std.ArrayList(ChatMessage) = .empty;
    try appendStatic(fixture.staticInput(), arena_state.allocator(), &messages);
    try appendTransient(fixture.transientInput(), arena_state.allocator(), &messages);
