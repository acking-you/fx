...
fn parseUsage(response: std.json.ObjectMap) types.Usage {
    ...
    return .{
        ...
        .cached_input_tokens = nestedUnsignedField(
            input_details,
            "cached_tokens",
        ),
        .cache_write_input_tokens = nestedUnsignedField(
            input_details,
            "cache_write_tokens",
        ),
        .reasoning_output_tokens = nestedUnsignedField(
            output_details,
            "reasoning_tokens",
        ),
    };
...
test "Responses usage projection retains optional cached and reasoning detail" {
    ...
    try std.testing.expectEqual(@as(?u64, 5), usage.cached_input_tokens);
    try std.testing.expectEqual(@as(?u64, 2), usage.cache_write_input_tokens);
    try std.testing.expectEqual(@as(?u64, 3), usage.reasoning_output_tokens);
}
