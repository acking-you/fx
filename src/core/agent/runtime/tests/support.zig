...
pub const FakeAgentRuntimeDeps = struct {
    ...
    fn refreshGatewayCredential(raw: *anyopaque, alloc: Allocator, source: types.CredentialSource, mode: runtime_deps.CredentialRefreshMode, expected_account_id: ?[]const u8) !?[]u8 {
        ...
        const refresh_index = self.credential_refresh_index;
        const token = self.credential_refresh_tokens[refresh_index];
        self.credential_refresh_index += 1;
        if (expected_account_id) |expected| {
            const actual = if (refresh_index < self.credential_refresh_account_ids.len)
                self.credential_refresh_account_ids[refresh_index]
            else
                null;
