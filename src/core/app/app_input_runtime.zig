...
test "app_input_runtime routes auth picker navigation before composer history" {
    ...
    try std.testing.expect((auth_runtime.Choice{ .action = .grok_login }).eql(app.auth.pickerView().selected_choice.?));
...
test "app_input_runtime onboarding Escape skips sign in for the session" {
    const alloc = std.testing.allocator;
    var app = try RoutingFakeApp.init(alloc);
    defer app.deinit();
...
test "app_input_runtime approval escape arrows move choices directly" {
    ...
    var app = try RoutingFakeApp.init(alloc);
    defer app.deinit();
    try std.testing.expect(try app.approval_prompt.syncRequest(alloc, .{ .label = "exec_command npm test" }));
...
test "app_input_runtime ctrl-o toggles full transcript while arrows preserve detail" {
...

    {
        ...
        try std.testing.expect(try approval_app.approval_prompt.syncRequest(alloc, .{ .label = "exec_command npm test" }));
