...
fn writeProblemsSummary(writer: *std.Io.Writer, app: anytype, alloc: std.mem.Allocator) !void {
    ...
    if (count == 0) try writer.writeAll("- no obvious errors captured in recent network, tool, or subagent state\n");
}
...
test "trace tool calls print errors first and mask obvious secrets" {
    ...
    failed.setArgs("{\"command\":\"OPENAI_API_KEY=abcdefghijklmnop zig build\"}");
