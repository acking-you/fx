...
* Model-facing shell execution has exactly one tool family: `exec_command` starts the process and `write_stdin` polls or interacts with it. Do not add command-shaped compatibility hooks, skill shortcuts, or a second executable backend. Historical `run_command` records may remain readable for session replay but must never become executable. Unified Exec output is projected through the normal queued TUI worker-event path and through stable ACP tool-call updates with structured input, stream identity, bounded accumulated output, and the final command result (`src/tools/shell/unified_exec.zig`, `src/core/execution/unified_exec.zig`, `src/core/tooling/tool_dispatch.zig`, `src/acp/prompt.zig`).
* Valid public `web_fetch` calls run by default without automatic safety review or a human approval prompt. Explicit allow, ask, and deny rules still take precedence, while URL validation and the public-network transport boundary remain enforced (`src/builtins/tools.zig`, `src/core/tooling/tool_admission.zig`, `src/tools/web/fetch_args.zig`).
