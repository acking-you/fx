...
* `/provider` owns TUI provider selection while `/login` owns authentication and credential selection. `/provider gateway <base-url> <api-key>` and ACP `fx/provider/configure` validate a Responses URL plus API key, persist that pair to `~/.fx/gateway-auth.json`, and activate Gateway immediately. Credential refresh, catalog loading, durable logout, cancellation, and worker reaping stay off the TUI event loop; logout cancels only an activation for the provider being removed, and `/logout gateway` deletes the persisted Gateway binding. Native ACP advertises `_meta.fx.providerControl` and supports provider switch, Codex and Grok login, and profile-persisted BYOK URL plus API-key configuration without blocking its read loop. Treat that custom URL and key as one binding: preserve it across temporary provider switches and use it for ordinary requests, automatic permission review, remote compaction, and persistent child turns. A background child builds its tool context only from its owned route snapshot until that turn ends, so connection reconfiguration cannot free or redirect it (`src/core/auth/gateway_session.zig`, `src/core/auth/provider_activation.zig`, `src/core/auth/provider_logout.zig`, `src/core/app/app_auth_runtime.zig`, `src/acp/server.zig`, `src/acp/prompt.zig`).
...
Project `.fx.json` accepts only repo-safe defaults: `sandbox`, `max_agent_steps`, `max_tool_result_bytes`, and `context`. Profile-owned keys such as `model`, `effort`, `fast_mode`, `slash_menu_categories`, `startup_scrollback`, `prompt_history`, `theme`, `statusLine`, `skill_match_fuzzy`, `first_call_tool_choice`, `permission_mode`, `credential_source`, and `permission` are ignored from project config before their values are parsed.
