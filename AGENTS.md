...
* `/provider` owns TUI provider selection while `/login` owns authentication and credential selection. Credential refresh, catalog loading, cancellation, and worker reaping stay off the TUI event loop. Native ACP advertises `_meta.fx.providerControl` and supports provider switch, Codex and Grok login, and connection-scoped BYOK URL plus API-key configuration without blocking its read loop. Treat that custom URL and key as one binding: preserve it across temporary provider switches and use it for ordinary requests, automatic permission review, and remote compaction (`src/core/auth/provider_activation.zig`, `src/core/app/app_auth_runtime.zig`, `src/acp/server.zig`, `src/acp/prompt.zig`).
