...
For Codex OAuth models, fx also consumes the model catalog's context-budget metadata. The effective context display reserves the model-declared headroom, defaulting to 95% of the raw window, and automatic compaction starts when the latest provider-reported total usage reaches the model limit, defaulting to 90% of the raw window. The TUI writes visible context notices when automatic compaction starts and settles; background remote compaction also owns the activity row until the replacement checkpoint is installed or the local fallback completes.
